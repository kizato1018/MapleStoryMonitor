...
3. **啟動程序**
   - 安裝完成後，可以用以下任一方式啟動：
     - **終端機啟動**：在程序資料夾中執行 `./run.command`
     - **雙擊啟動**：雙擊 `run.command` 檔案（程序啟動後可關閉終端機視窗）

4. **重要權限設定（首次啟動時可能需要）**
   - macOS 可能會要求「輔助使用」權限來擷取螢幕
   - 如果程序無法正常工作，請前往：
     - 「系統偏好設定」→「安全性與隱私」→「隱私權」
     - 選擇「輔助使用」或「螢幕錄製」
     - 將 Python 或終端機添加到允許清單中
