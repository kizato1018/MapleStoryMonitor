...
如果您的遊戲介面使用特殊符號，可以修改 OCR 允許清單：

1. 在「設定」標籤頁找到「OCR 允許字元」
2. 添加您需要的字符，例如：`0123456789.[]/%+-`

### 配置檔案說明

程序會自動儲存設定到 `game_monitor_config.json` 檔案：
...
```json
{
  ...
  "tabs": {
    "HP": {"x": 377, "y": 705, "w": 90, "h": 18},
    "MP": {"x": 542, "y": 705, "w": 90, "h": 18},
    "EXP": {"x": 730, "y": 704, "w": 117, "h": 21}
  }
}
```
...
程序運行記錄儲存在 `Log/game_monitor.log`，如遇問題可查看詳細錯誤訊息。
