...
class OCREngine:
    ...
    def _preprocess_potion_image(self, image: np.ndarray) -> np.ndarray:
        ...
        image = image[int(h * top):int(h * (1-bottom)), int(w * left):int(w * (1-right))]
        ...
        kernel = np.ones((3,3), np.uint8) if min(image.shape[0], image.shape[1]) > 100 else np.ones((2,2), np.uint8)
        ...
        # 其餘區域保留原樣
        output = floodfilled
        kernel = np.ones((4,4), np.uint8) if min(image.shape[0], image.shape[1]) > 100 else np.ones((3,3), np.uint8)
        ...
        min_height = image.shape[0] * 0.55
        max_width = image.shape[1] * 0.25
